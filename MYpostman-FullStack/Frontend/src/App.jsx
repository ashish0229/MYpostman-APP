...
export default function App() {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'moderation': return <ModerationLogView logs={liveLogs} />;
...
// --- AUTH VIEW (with display name and logo) ---
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-100">
            <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-2xl shadow-lg">
                <img src="/logo.png" alt="MYpostmate Logo" className="mx-auto h-16 w-auto" />
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                    {isRegistering && (
                        <div>
                            <label htmlFor="displayName" className="text-sm font-bold text-gray-600 block">Display Name</label>
                            <input id="displayName" type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)} className="w-full p-3 mt-2 text-gray-700 bg-gray-50 rounded-lg border focus:outline-none focus:ring-2 focus:ring-yellow-500" required />
                        </div>
                    )}
                    <div>
                        <label htmlFor="username" className="text-sm font-bold text-gray-600 block">Username (for login)</label>
                        <input id="username" type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full p-3 mt-2 text-gray-700 bg-gray-50 rounded-lg border focus:outline-none focus:ring-2 focus:ring-yellow-500" required />
                    </div>
                    <div>
                        <label htmlFor="password"  className="text-sm font-bold text-gray-600 block">Password</label>
                        <input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-3 mt-2 text-gray-700 bg-gray-50 rounded-lg border focus:outline-none focus:ring-2 focus:ring-yellow-500" required />
                    </div>
                    {isRegistering && (
                         <div>
                            <label htmlFor="role" className="text-sm font-bold text-gray-600 block">Role</label>
                             <select id="role" value={role} onChange={e => setRole(e.target.value)} className="w-full p-3 mt-2 text-gray-700 bg-gray-50 rounded-lg border focus:outline-none focus:ring-2 focus:ring-yellow-500">
                    ...
                    <button type="submit" disabled={isLoading} className="w-full flex justify-center py-3 px-4 rounded-lg shadow-sm font-medium text-gray-900 bg-yellow-400 hover:bg-yellow-500 disabled:bg-gray-400">
                        {isLoading ? 'Processing...' : (isRegistering ? 'Register' : 'Sign In')}
                    </button>
                </form>
                 <div className="text-center">
                    <button onClick={() => { setIsRegistering(!isRegistering); setError(null); }} className="text-sm text-yellow-600 hover:underline">
    ...
    <aside className="w-64 bg-white p-6 flex flex-col justify-between shadow-lg">
        <div>
            <div className="flex items-center mb-10">
                <img src="/logo.png" alt="MYpostmate Logo" className="h-8 w-auto mr-2" />
                <h1 className="text-2xl font-bold text-yellow-500">MYpostmate</h1>
            </div>
            <nav>
                <ul>
                    <NavItem icon={<LayoutDashboardIcon />} label="Dashboard" tabName="dashboard" activeTab={activeTab} setActiveTab={setActiveTab} />
                    <NavItem icon={<BotIcon />} label="Generator" tabName="generator" activeTab={activeTab} setActiveTab={setActiveTab} />
                    <NavItem icon={<HistoryIcon />} label="History" tabName="history" activeTab={activeTab} setActiveTab={setActiveTab} />
                    <NavItem icon={<FileStackIcon />} label="Moderation Log" tabName="moderation" activeTab={activeTab} setActiveTab={setActiveTab} />
                    {user?.role === 'admin' && (
                        <>
                            <NavItem icon={<UsersIcon />} label="Users" tabName="users" activeTab={activeTab} setActiveTab={setActiveTab} />
                            <NavItem icon={<AnalyticsIcon />} label="Analytics" tabName="analytics" activeTab={activeTab} setActiveTab={setActiveTab} />
                        </>
                    )}
                    <NavItem icon={<CalendarIcon />} label="Calendar" tabName="calendar" activeTab={activeTab} setActiveTab={setActiveTab} />
        ...
        <div className="border-t pt-4">
             ...
             <button onClick={onLogout} className="w-full flex items-center justify-center p-2 rounded-lg text-gray-600 hover:bg-red-100 hover:text-red-700">
...
const HistoryView = () => { const [posts, setPosts] = useState([]); useEffect(() => { const fetchHistory = async () => { try { const data = await apiClient.request('/posts'); setPosts(data); } catch (error) { console.error("Failed to fetch post history:", error); } }; fetchHistory(); }, []); return ( <div> <h2 className="text-2xl font-bold mb-6">Post Generation History</h2> <div className="space-y-4"> {posts.map(post => ( <div key={post.id} className="bg-white p-4 rounded-lg shadow border"> <p className="font-bold">{post.topic}</p> <p className="text-sm text-gray-600 mt-2 whitespace-pre-wrap">{post.content}</p> <div className="text-xs text-gray-500 mt-2">Status: {post.status} | Created: {new Date(post.created_at).toLocaleString()}</div> </div> ))} </div> </div> ); };
const ModerationLogView = ({ logs }) => ( <div> <h2 className="text-2xl font-bold mb-6">Live Moderation Log</h2> <div className="bg-white p-6 rounded-xl shadow-lg border overflow-x-auto"> <table className="min-w-full"> <thead> <tr className="border-b"><th className="text-left p-2">Post ID</th><th className="text-left p-2">Action</th><th className="text-left p-2">Reason</th><th className="text-left p-2">Timestamp</th></tr> </thead> <tbody> {logs.map(log => ( <tr key={log.id} className="border-b hover:bg-gray-50"> <td className="p-2">{log.post_id}</td> <td className="p-2"><span className={`px-2 py-1 rounded-full text-xs font-semibold ${ log.action === 'rejected' ? 'bg-red-100 text-red-800' : log.action === 'quarantined' ? 'bg-orange-100 text-orange-800' : log.action === 'published' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }`}>{log.action}</span></td> <td className="p-2">{log.reason}</td> <td className="p-2">{new Date(log.created_at).toLocaleString()}</td> </tr> ))} </tbody> </table> </div> </div> );
